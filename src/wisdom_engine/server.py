...
from .responses import BoardResult, CaseList, CasePage, CaseResult, CheckComparison, MutationResult
...
def list_cases(offset: Annotated[int, Field(ge=0)] = 0,
               limit: Annotated[int, Field(ge=1, le=100)] = 50) -> CaseList:
    """Discover saved inquiries in stable creation order."""
    return CaseList.model_validate(store().list_cases(offset, limit))


@mcp.tool()
def open_case(case: CaseInput, request_id: Identifier) -> CaseResult:
    """Open an inquiry. Scope names the subject, setting, and relevant time."""
    return CaseResult.model_validate(store().open_case(case, request_id))


@mcp.tool()
def record_observation(case_id: Identifier, observation: Observation,
                       expected_sequence: Sequence, request_id: Identifier) -> MutationResult:
    """Record a sourced report/result, never a model prediction or invented measurement."""
    return MutationResult.model_validate(store().append(case_id, [observation], expected_sequence, request_id))


@mcp.tool()
def add_claim(case_id: Identifier, claim: Claim, expected_sequence: Sequence, request_id: Identifier) -> MutationResult:
    """Add an explanation, assumption, interpretation, or belief driver. Multiple causes may coexist."""
    return MutationResult.model_validate(store().append(case_id, [claim], expected_sequence, request_id))
...
def add_prediction(case_id: Identifier, prediction: Prediction,
    ...
    return MutationResult.model_validate(store().append(case_id, [prediction], expected_sequence, request_id))
...
def record_assessment(case_id: Identifier, assessment: Assessment,
    ...
    _manual(assessment)
    return MutationResult.model_validate(store().append(case_id, [assessment], expected_sequence, request_id))
...
def record_batch(case_id: Identifier, records: Annotated[list[RecordPayload], Field(min_length=1, max_length=20)],
    ...
    for record in records:
        _manual(record)
    return MutationResult.model_validate(store().append(case_id, records, expected_sequence, request_id))
...
def revise_record(case_id: Identifier, record_id: Identifier, expected_revision: Revision,
                  ...
                  replacement: RecordPayload | None = None, withdraw: bool = False) -> MutationResult:
    """Replace content or withdraw it. Dependent assessments become stale, never silently reused."""
    if replacement:
        _manual(replacement)
    return MutationResult.model_validate(store().revise(case_id, record_id, expected_revision, expected_sequence, request_id,
                          reason, replacement, withdraw))


@mcp.tool()
def get_case(case_id: Identifier, offset: Annotated[int, Field(ge=0)] = 0,
             limit: Annotated[int, Field(ge=1, le=100)] = 50) -> CasePage:
    ...
    return CasePage.model_validate({**snapshot.model_dump(exclude={'records'}),
...
@mcp.tool()
def inquiry_board(case_id: Identifier, omitted_source_group: Text | None = None) -> BoardResult:
    """Show evidence statuses, unknown coverage, and invalidations; optionally test source sensitivity."""
    return BoardResult.model_validate(board(store().snapshot(case_id), omitted_source_group))


@mcp.tool()
def add_check(case_id: Identifier, check: Check, expected_sequence: Sequence, request_id: Identifier) -> MutationResult:
    """Record a proposed procedure and scenario outcomes. This tool does not execute the check."""
    return MutationResult.model_validate(store().append(case_id, [check], expected_sequence, request_id))


@mcp.tool()
def compare_checks(case_id: Identifier) -> CheckComparison:
    """Compare declared check tradeoffs; no probability or automatic execution is implied."""
    return CheckComparison.model_validate(_compare_checks(store().snapshot(case_id)))
...
def record_decision(case_id: Identifier, decision: Decision,
    ...
    return MutationResult.model_validate(store().append(case_id, [decision], expected_sequence, request_id))
...
def resolve_case(case_id: Identifier, resolution: Resolution,
    ...
    return MutationResult.model_validate(store().append(case_id, [resolution], expected_sequence, request_id))


@mcp.tool()
def export_case(case_id: Identifier) -> Bundle:
    """Export the complete revision history as portable JSON; no provider credentials are included."""
    return Bundle.model_validate(store().export_case(case_id))


@mcp.tool()
def import_case(bundle: Bundle, request_id: Identifier) -> CaseResult:
    """Replay/validate an archive into an absent case ID. Attribution is preserved, not authenticated."""
    return CaseResult.model_validate(store().import_case(bundle.model_dump(), request_id))
...
async def assess_evidence(case_id: Identifier, pairs: Annotated[list[Pair], Field(min_length=1, max_length=16)],
    ...
    return MutationResult.model_validate(await _assess_evidence(store(), client, case_id, pairs, expected_sequence, request_id))
